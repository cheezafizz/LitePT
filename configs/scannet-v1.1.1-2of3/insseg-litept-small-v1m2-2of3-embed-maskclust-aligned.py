...
# ALIGNED-mask variant of insseg-litept-small-v1m2-2of3-embed-maskclust.py.
# Same 2D-mask cannot-link clustering constraint (cluster_mask_constraint=True): when
# per-point mask labels are supplied (the GT-free VGGT inference path, tools/infer_insseg.py),
# each geometric (offset+BFS) proposal — already sub-divided in embedding space — is cut so
# that two points coming from the SAME camera view but lying in DIFFERENT D-FINE-seg 2D
# instance masks never end up in the same predicted instance. Points from different views
# still merge freely, so an object spanning views is not shattered.
#
# The ONLY difference vs -maskclust is the mask source this run is meant to consume:
# the per-point labels in data/scannet-v1.1.1-2of3-vggt-mask-aligned/ baked from the
# ALIGNED COCO masks (/home/fai/workspace/jhp/dataset/val_sample_output_aligned, 518x392,
# already in the VGGT image grid → built with --no-mask-undistort --no-mask-center-pp,
# see tools/build_mask_labels_aligned.sh) — NOT the older 640x480 masks under the plain
# -vggt-mask dataset. The clustering params are identical to -maskclust; the mask source
...
# (standard ScanNet train/val has no 2D masks). Clean A/B vs embed.
...
save_path = "exp/scannet/insseg-litept-small-v1m2-2of3-embed-maskclust-aligned"
...
model = dict(
    ...
    # (auto-fed by tools/infer_insseg.py on VGGT scenes carrying mask_instance.npy).
