...
# 2D-mask-constrained-clustering variant of insseg-litept-small-v1m2-2of3-embed.py.
# Identical to embed EXCEPT it enables a per-point 2D-mask cannot-link constraint in
# the inference clustering (cluster_mask_constraint=True). When per-point mask labels
# are supplied (the GT-free VGGT inference path, tools/infer_insseg.py), each geometric
# (offset+BFS) proposal — already sub-divided in embedding space — is additionally cut
# so that two points coming from the SAME camera view but lying in DIFFERENT D-FINE-seg
# 2D instance masks never end up in the same predicted instance. Points from different
# views still merge freely, so an object spanning views is not shattered.
#
...
save_path = "exp/scannet/insseg-litept-small-v1m2-2of3-embed-maskclust"
