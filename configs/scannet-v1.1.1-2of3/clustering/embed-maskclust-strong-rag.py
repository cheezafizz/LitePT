...
# into instances.
#
# Variant: GEOMETRIC + EMBEDDING SPLIT + 2D-MASK CANNOT-LINK (multi-view, STRONG) + RAG HISTOGRAM MERGE.
...
#
# Superset of clustering/embed-maskclust-strong.py: the ONLY added behavior is the post-clustering
# Region-Adjacency-Graph merge. The strict cannot-link is the MOST aggressive splitter -- it cuts
# every same-view/distinct-mask pair transitively, so a single noisy 2D-mask label fragments a real
# object into several small instances at touching-object seams. This config adds the inverse step:
...
# Because the histogram aggregates hundreds of points, the lone noisy point that broke the cluster
# is mathematically drowned out by the majority, so the fragments re-merge -- while two genuinely-
# distinct touching objects (different dominant masks) keep dissimilar histograms and stay apart.
# The merge runs BEFORE the cluster_propose_points size filter, so re-merged fragments can clear the
# size gate. See models/point_group/point_group_v1m2_custom_criteria.py
# (_merge_proposals_by_mask_histogram). "Strict separates maximally, then RAG re-merges what agrees."
# Clean A/B vs clustering/embed-maskclust-strong.py (RAG merge OFF).
#
