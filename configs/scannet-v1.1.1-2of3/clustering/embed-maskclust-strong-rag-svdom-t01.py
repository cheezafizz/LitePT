# AUTO-GENERATED rag_merge_thresh sweep variant of clustering/embed-maskclust-strong-rag-svdom.py.
# Identical to that config EXCEPT rag_merge_thresh=0.1 (shared-view conflict metric:
# LOWER thresh -> stricter -> fewer, larger merges). See embed-maskclust-strong-rag-svdom.py
...
cluster = dict(
    ...
    rag_merge_thresh=0.1,            # MAX different-mask ratio that still merges (lower = stricter = fewer merges)
    rag_adjacency_radius=2.5,        # RAG adjacency radius (voxel units); = mask_split_radius -> 5 mm
    rag_sim_metric="shared_view_dominant",
)
