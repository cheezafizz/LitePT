...
save_path = "exp/scannet/insseg-litept-small-v1m2-2of3-rigidaug"
...
data = dict(
    ...
    train=dict(
        ...
        transform=[
            ...
            # Rigid-only variant: RandomJitter (5 mm per-point noise) and
            # ElasticDistortion (non-rigid warp) are intentionally OMITTED. Instances
            # here sit <=1 cm apart on a 2 mm voxel grid; both transforms move points
            # relative to their neighbors and blur instance boundaries, which inference
            # never does. All remaining geometry aug is rigid / isotropic-similarity
            # (RandomScale is isotropic) and preserves each point's relative position.
